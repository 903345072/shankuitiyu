...
import 'package:jingcai_app/pages/score_pages/socreFoot/allBasket.dart';
import 'package:jingcai_app/pages/score_pages/socreFoot/allFoot.dart';
import 'package:jingcai_app/pages/score_pages/socreFoot/hotBasket.dart';
...
class basketScreen extends StatefulWidget {
  String date_;
  basketScreen({required this.date_});
  @override
  State<StatefulWidget> createState() {
    // TODO: implement createState
    return _basketScreen();
  }
}

class _basketScreen extends State<basketScreen> with TickerProviderStateMixin {
  late TabController _tabC;
  List<GetGuess> hot_list = [];
  List<AllMatched> all_list = [];
  List<GetGuess> guess_list = [];
  ...
  Future _loadArticle() async {
    G.api.gameAdd.getBasketLeagues({}).then((value) {
      var d = BasketScreenModel.fromJson(value);
      final data = d.data;
      if (data != null) {
        setState(() {
  ...
  setAllcheck() {
    ...
    if (_tabC.index == 1) {
      all_list.forEach((element) {
        List<basketElement>? s = element.list;
        s?.forEach((element1) {
